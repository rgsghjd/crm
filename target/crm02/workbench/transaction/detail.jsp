<%@ page import="cg.software.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="cg.software.workbench.domain.Tran" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%
String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/";

	List<DicValue> dicList= (List<DicValue>) application.getAttribute("stageList");//取到阶段的字典值列表
	Map<String,String> pMap= (Map<String, String>) application.getAttribute("stagePossibility");//取得阶段可能性对应关系
	Set<String> stageList=pMap.keySet();//取得阶段列表
	//取得阶段分界点的下标
	int point=0;
	for (int i=0;i<dicList.size();i++){
		DicValue dvalue=dicList.get(i);
		String stage=dvalue.getValue();//当前阶段
...
<script type="text/javascript">
	...
	$(function(){
		...
		showStageHistory("${t.id}");
	});
	function showStageHistory(id) {
		$.ajax({
			url:"workbench/transaction/stageList.do",
			data:{
				"id":id
			},
			dataType:"json",
	...
	function changeIco(stage,index) {


		var point="<%=point%>"//阶段的分界点
		var currenPossibility=$("#possibility").html();


			if ("0"==currenPossibility){
				//前面七个黑圈，后面两个一个红叉一个黑叉
				for(var i=0;i<point;i++){
						//黑圈------
					$("#"+i).removeClass();//清除原有样式
					$("#"+i).addClass("glyphicon glyphicon-record mystage");
					$("#"+i).css("color","#000000")

					}
				for (var i=point;i<<%=dicList.size()%>;i++){
					if (i==index){
						//红叉
						$("#"+i).removeClass();//清除原有样式
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		//取得当前阶段可能性
			...
			String currenstage= t.getStage();
			String currenPossibility=pMap.get(currenstage);
			if ("0".equals(currenPossibility)){
				//前七个为黑圈，后两个一个红叉一个黑叉
				for (int i=0;i<dicList.size();i++){
					DicValue dicValue=dicList.get(i);
					String stage=dicValue.getValue();
					String possibility=pMap.get(stage);
					if("0".equals(possibility)){
						//遍历出来的可能性为零说明是后两个
						if (currenstage.equals(stage)){
							//为零阶段是当前阶段，标记为红叉
							//红叉--------------
		...
		<%
				...
				//前七个可能是绿圈，绿标，黑圈，后两个一定是黑叉
				int index=0;//记录当前阶段的下标,用来标记绿标
				for (int i=0;i<dicList.size();i++){
					DicValue dicValue=dicList.get(i);
					String stage=dicValue.getValue();
					if (stage.equals(currenstage)){
						index=i;
						break;
					}
				}
